...
function MapHeader(props) {
  ...
  const createNewWaypoint = (id, label) => ({id, value: '', label});

  const [waypoints, setWaypoints] = useState({
    0: createNewWaypoint(0, 'From'),
    1: createNewWaypoint(1, 'To'),
  });
  ...
  const renderFieldItem = (item) => {
    ...
    return (
      <TextInput
        ...
        onFocus={() =>
          props.navigation.navigate('PlaceInput', {
            id,
            onPlaceChosen: editField,
          })
        }>
        {value}
      </TextInput>
